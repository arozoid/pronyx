...
impl PRoot {
    pub fn new(config: Config) -> PRoot {
        PRoot {
            config: config,
            ...
            .map(|arg| {
                CString::new(arg.as_bytes()).map_err(|_| {
                    crate::errors::Error {
                        ...
                        msg: Some(Box::new(format!("illegal parameters: {}", arg))),
                        source: None,
                    }
                })
            })
        ...
        match unsafe { unistd::fork() }.context("Failed to fork() when starting process")? {
            ...
            ForkResult::Child => {
                let init_child_func = || -> Result<()> {
                    ...
                    unsafe {
                        kill(unistd::getpid().as_raw(), Signal::SIGSTOP as i32);
                    }
                    //TODO: seccomp
...
mod tests {
    ...
    fn create_proot_and_tracee() {
        let config = Config { 
            root_id: false, link2symlink: false
        };
        let fs = FileSystem::new();
        let mut proot = PRoot::new(config);
