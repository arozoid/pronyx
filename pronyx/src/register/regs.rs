...
impl Registers {
    ...
    /// `get_reg!` macro.
    #[inline]
    pub fn get_raw(&self, raw_regs: &RegisterSet, register: Register) -> Word {
...
mod tests {
    ...
    fn test_fetch_regs_sysnum_sleep_test() {
        let rootfs_path = get_test_rootfs_path();
        use crate::sc::nr::{NANOSLEEP, CLOCK_NANOSLEEP};
        ...
        fork_test(
            ...
            |tracee, _| {
                ...
                tracee.regs.get_sys_num(Current) == NANOSLEEP.try_into().unwrap()
                    || tracee.regs.get_sys_num(Current) == CLOCK_NANOSLEEP.try_into().unwrap()
            },
            // child
            || {
                // calling the sleep function, which should call the NANOSLEEP syscall
    ...
    fn test_push_regs_void_sysnum_sleep_test() {
        ...
        fork_test(
            ...
            |tracee, _| {
                ...
                if sys_num == NANOSLEEP.try_into().unwrap() || sys_num == CLOCK_NANOSLEEP.try_into().unwrap() {
