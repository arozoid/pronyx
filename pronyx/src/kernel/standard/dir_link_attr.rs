...
pub fn enter(tracee: &mut Tracee) -> Result<()> {
    ...
    let deref_final = match sys_num as i64 {
        ...
        crate::sc::nr::UNLINK | crate::sc::nr::RMDIR | crate::sc::nr::MKDIR => false,
        _ => {
        ...
        test_with_proot(
            ...
            || {
                ...
                let result = std::panic::catch_unwind(|| {
                    ...
                    // `linkpath`.
                    assert_eq!(
                        nc::lgetxattr(linkpath, attr_name, &mut buf),
                        Err(nc::ENODATA)
                    );
                    nc::lgetxattr(filepath, attr_name, &mut buf)
                        .unwrap();
                    ...
                    let attr_len = nc::llistxattr(linkpath, &mut buf)
                        .unwrap() as usize;
                    // `attr_len` should be `0` because there are no attributes.
                    assert_eq!(attr_len, 0);
                    let attr_len = nc::llistxattr(filepath, &mut buf)
                        .unwrap() as usize;
